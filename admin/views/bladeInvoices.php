...
if( $orders = selectDB("orders2","`id` != 0 {$type} GROUP BY `orderId` ORDER BY `date` DESC") ){
    ...
    for( $i = 0; $i < sizeof($orders); $i++ ){
		...
		$method = ( in_array($orders[$i]["paymentMethod"],array_keys($paymentMethod)) ) ? direction($paymentMethodTitle[$orders[$i]["paymentMethod"]],$paymentMethodTitle[$orders[$i]["paymentMethod"]]) : "";
